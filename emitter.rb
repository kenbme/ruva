class Emitter
  ...
  def start_elsif(condition)
    emit "else if (ruva_truthy(#{condition}))"
    start_block
  end
  ...
  def start_while(condition)
    emit "while (ruva_truthy(#{condition}))"
    start_block
  end
  ...
  def ruva_math(op, left, right)
    emit "ruva_#{op}(#{left}#{separator_str}#{right});"
  end
  ...
  def ruva_math_str(op, left, right)
    "ruva_#{op}(#{left}#{separator_str}#{right})"
  end
